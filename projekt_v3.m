...
originalImage=imresize(originalImage,[1024,1024]);

%ptsOriginal = detectSURFFeatures(rgb2gray(originalImage));

originalImagenoise=imnoise(originalImage,"speckle",0.01);
...
%  originalImagenoise = imfilter(originalImagenoise, gaussianFilter);
 %originalImagenoise = 255 - originalImage;
 %originalImagenoise=flip(originalImage,2);
...
for i = 1:gridRows
    ...
    if sum(reconstructedImage(maxy+round(0.2*SubImSize(1)):maxy+round(SubImSize(1)*0.8), maxx+round(0.2*SubImSize(2)):maxx+round(0.8*SubImSize(2)),1),"all")==0
        ...
        reconstructedimage1=reconstructedImage;
        reconstructedimage1(maxy:maxy+SubImSize(1)-1, maxx:maxx+SubImSize(2)-1,:)=subImageRotate;
        xd1=corr2(rgb2gray(originalImage),rgb2gray(reconstructedimage1));
        xd2=corr2(rgb2gray(originalImage),rgb2gray(reconstructedImage));
        if xd2<xd1
